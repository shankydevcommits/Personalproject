...
### Tasmania state notes included. Core rules are federal — read this before you do anything else.
...
## Section 5 — Tasmania-Specific Considerations

- [ ] **Check Tasmania's land tax settings for SMSF-held property.** Tasmania's land tax threshold is **$124,999** (increased from $99,999 from 1 July 2024) — SMSF property is typically held via a bare trust (LRBA) or corporate trustee, so confirm with your adviser how your fund's specific structure is assessed.
- [ ] **Check the 8.0% foreign purchaser surcharge** if any fund member could be considered a foreign purchaser for these purposes.
- [ ] **Remember Tasmania has no statutory cooling-off period at all**, similar to WA — all finance, inspection, and bare trust arrangements need to be genuinely resolved before your SMSF trustee signs, not after.
- [ ] **Standard Tasmanian stamp duty applies to SMSF property purchases** — there's no concession available to a superannuation fund, and note that Tasmania's first-home-buyer duty settings (which changed substantially around 30 June 2026) are irrelevant here regardless, since a super fund was never eligible for first-home-buyer concessions. See the separate Property Investment Purchase Checklist (Tasmania) in this series for the general investor duty and land tax framework.
- [ ] **Engage a Tasmanian conveyancer or solicitor experienced specifically with SMSF/bare trust transactions** — correctly naming the trustee structure in the contract is a common error point, and there's no cooling-off period to fix it after signing if it's wrong.
