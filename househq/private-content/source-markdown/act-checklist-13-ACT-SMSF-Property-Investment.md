...
### ACT state notes included. Core rules are federal — read this before you do anything else.
...
## Section 5 — ACT-Specific Considerations

- [ ] **Check ACT land tax settings for SMSF-held property.** The ACT applies land tax to any property that isn't a principal place of residence — since an SMSF is never a natural person occupying the property, **land tax will generally apply to any SMSF-held residential property in the ACT**, in addition to standard rates. Confirm current settings with the ACT Revenue Office for your fund's specific structure.
- [ ] **Check the 7% foreign investor duty surcharge** if any fund member could be considered a foreign person for these purposes.
- [ ] **Understand ACT's new stamp duty settings don't help your SMSF.** The ACT's 1 July 2026 full stamp duty abolition applies specifically to first home buyer *owner-occupiers* and new unit-titled property buyers — a superannuation fund purchasing an investment property doesn't qualify, and standard conveyance duty applies.
- [ ] **Remember ACT property is leasehold (Crown lease), not freehold** — your SMSF's solicitor needs to confirm the bare trust and lease documentation align correctly, which adds a layer of complexity specific to the ACT.
- [ ] **Engage an ACT solicitor experienced specifically with SMSF/bare trust transactions** — correctly naming the trustee structure in the contract is a common error point.
